...
class VimLexer extends DefaultLexer
{
    ...
    protected $state_table = [
        ...
        VIM_STRLIT_S => ['\\\\' => VIM_ESCAPE_S, '\'' => VIM_END_QUOTE, "[\r\n]" => VIM_NORMAL],
        VIM_STRLIT_D => ['"' => VIM_END_QUOTE],
        ...
        VIM_COMM => ["[\r\n]" => VIM_NORMAL],
        VIM_ESCAPE_S => ['\'' => VIM_STRLIT_S, VIM_ESC_INV_S],
        VIM_ESCAPE_D => ['["nrt\\\]' => VIM_STRLIT_D, VIM_ESC_INV_D],
        VIM_ESC_INV_D => [VIM_STRLIT_D],
        VIM_ESC_INV_S => [VIM_STRLIT_S],
    ];
     ...
     *                               Used when tokenizer has left to tokenize another language then returns back here
     *                               to determine which state to begin in
     *
     * @return array Array of (TOKEN => string)
     */
    public function tokenize($output, &$starting_state = null)
    {
        ...
        while (isset($output[$i])) {
            ...
            if ((!isset($output[$i]) || preg_match("/^[\r\n]$/", $char)) && $state === VIM_STRLIT_D) {
                ...
                $state = VIM_NORMAL;
                $ret_tokens[] = ['token' => $this->tokens[VIM_COMM], 'string' => $cur_state_string];
                $cur_state_string = '';
                continue;
            }
    ...
    /**
     * Custom handler for VIM_PARAM tokens.
     *
     * @param string $string
     * @param string $prev   Previous token
     *
     * @return array
     */
    ...
    /**
     * Custom handler for VIM_TCMD tokens.
     *
     * @param string $string
     * @param string $prev   Previous token
     *
    ...
    {
        return self::_handleKeyword(['commands' => 'VIM_CMD'], $string);
    }

    /**
     * Iterate over the commands, keywords, etc. and determine which type of token it should use.
     *
     * @param array  $tests  Array of ('variable' => 'TOKEN')
