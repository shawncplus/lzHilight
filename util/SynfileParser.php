...
class SynfileParser
{
    ...
    private static $valuerange = [0x00, 0x5F, 0x87, 0xAF, 0xD7, 0xFF];
        ...
        foreach ($synfile as $line_no => $line) {
            ...
            elseif (preg_match('/^#[0-9A-F]{3,6}(%[ib])?/i', $color)) {
                ...
                if ($bg_color !== null && preg_match('/^#?[0-9A-F]{3,6}/i', $bg_color)) {
                    ...
                    $color_map[$token] = ['fg' => $color, 'bg' => $bg_color, 'decorators' => $decorators];
                    continue;
                } elseif ($html_col && $decorators !== '') {
                    $color_map[$token] = ['fg' => $color, 'decorators' => $decorators];
                    continue;
                }
            } // Token link
    ...
    public static function htmlToSgr($color)
    {
        ...
        $colparts = array_map('hexdec', [substr($color, 0, 2), substr($color, 2, 2), substr($color, 4, 2)]);
    ...
    public static function xterm2rgb($color)
    {
        ...
        if ($color >= 16 && $color <= 232) {
            $color -= 16;
            $rgb = [self::$valuerange[($color / 36) % 6], self::$valuerange[($color / 6) % 6], self::$valuerange[$color % 6]];
        }
